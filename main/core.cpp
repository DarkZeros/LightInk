...
// ============================================================
// Helper: build a WatchfaceState from the current core state
// ============================================================
static WatchfaceState makeState(const Core& core) {
    WatchfaceState s;
    s.mTime    = core.mNow;
    s.mBattery = core.mBattery.mCurPercent;
    return s;
}

// Helper: advance a tmElements_t by one minute (wraps hours/days simply)
static void advanceOneMinute(tmElements_t& t) {
    t.Minute++;
    if (t.Minute >= 60) {
        t.Minute = 0;
        t.Hour++;
        if (t.Hour >= 24) {
            t.Hour = 0;
            // Day/month/year rollover not needed for short lookahead
        }
    }
}
